...
| **2b.5b-part2-followup (mostly landed)** | (1) **Landed.** Per-rollout metadata threading: `HyWorldPlayCtrl` gains `rollout_viewmats` / `rollout_Ks` / `rollout_action` slots, populated by `HyWorldPlayWanCtrlEncoder.forward` from the full-trajectory tensors. `HyWorldPlayWan21Transformer._slice_per_frame` (the parity-incorrect stub) is replaced by `_index_rollout_buffer`, which uses `tensor.index_select(axis, memory_frame_indices)` on the rollout buffer when bound. Patchify rebuild passes the new fields through unchanged. CPU tests pin defaults / patchify survival / encoder attach / unbound-conditioner fallback. (2) **Landed.** GPU smoke on RTX 6000 Pro at 256x448 / 2-chunk: pipeline boots, distilled checkpoint loads, prefill executor fires on chunk 1 with synthetic memory indices, `HyWorldPlayMemoryKVCache` populates per block, dual-branch attention concats memory + current K/V, mp4 written. Surfaced four real bugs that CPU tests couldn't catch: (a) `wan22_ti2v_5b_vae_state_dict_transform` was missing the `mid_block.resnets.{0,1}` -> `middle.{0,2}.residual.{0,2,3,6}` per-field remap (12 VAE params per side stayed on `meta`); (b) `_native_runner._bind_camera_data` needed `unsqueeze(0)` on viewmats / Ks for `prope_qkv`'s `[batch=1, cameras, 4, 4]` rank; (c) `_compute_memory_indices` needed an explicit fp32 cast before `.numpy()` for bf16-bound viewmats; (d) `_native_runner.run` needed to cast the preprocessed first-frame tensor to the pipeline dtype. Two known quirks observed: prefill executor fires once per denoising step rather than once per chunk (idempotent so correct, but wastes `N-1` prefill passes per chunk), and upstream's `select_mem_frames_wan` has a `historical_clip_starts` boundary issue with short rollouts (the GPU smoke patches around it via a synthetic indices list). (3-5) **Still pending**: end-to-end parity diff at production resolution (704x1280, 8-chunk) vs phase-1 vendor-wrapper baseline; parity sub-venv removal (`sageattention`, `cloudpickle`, `accelerate`, `transformers==4.57.6`); `--use-native-pipeline` default flip. | ~400 landed | 2b.5b-part2 |
...
| 2b.5b-part2-followup | (1) **Landed.** Per-rollout `viewmats` / `Ks` / `action` buffers threaded from `HyWorldPlayWanCtrlEncoder` through the ctrl to the prefill driver; `_slice_per_frame` stub replaced by `_index_rollout_buffer` (parity-correct path with `tensor.index_select(axis, memory_frame_indices)`). (2-4) **Still pending.** GPU smoke with the distilled checkpoint surfaces any structural bugs the CPU tests can't catch. Full pipeline parity diff matches the documented phase-1 parity bar (mean per-frame uint8 RGB delta target: ≤ the phase-1 threshold from `tests/parity_check/README.md`). Parity sub-venv removed. Default flipped to native. |

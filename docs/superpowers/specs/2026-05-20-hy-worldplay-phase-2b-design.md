...
| **2b.6 (new follow-on)** | Root-cause + close the HY-swap I2V conditioning divergence surfaced by 2b.5b-part2-followup item (3). Working hypothesis (to confirm with a focused probe before coding): the HY transformer subclass tree silently overrides either (a) the I2V mask / clean-latent stamping that `Wan21Transformer` does at chunk 0, (b) the first-frame-per-token timestep masking (`ti2v_first_frame_per_token_timestep=True`), or (c) a clean-vs-noisy split inside `predict_flow` that the dual-branch attention rewrite assumed-but-didn't-preserve. Acceptance bar is the same `mean |Δ| <= 5 / 255` parity threshold against the phase-1 vendor wrapper, after which the deferred 2b.5b-part2-followup cleanup (sub-venv removal + default flip) lands as the last step of 2b.6. The parity diff harness from 2b.5b-part2-followup (vendor `wan/generate.py` invocation + `imageio[FFMPEG]`-based per-frame uint8 RGB delta) is reusable as-is. | ~200-600 LoC depending on root cause | 2b.5b-part2-followup |

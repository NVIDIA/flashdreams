...
application configs support these tested aspect-compatible examples:
...
- The public LightVAE single-view recipe used by the application configs supports
...
The perf application config uses the OmniDreams single-view native CUDA extension for the
DiT path:
...
``sage3_fp8``; the perf application config pins ``cudnn``. Setting the network
``self_attention_backend`` to ``sage2`` also selects Native Sage2 when native
DiT acceleration is enabled; text cross-attention remains on cuDNN.
...
cuDNN path. Sage3 and Sage3 FP8 remain Blackwell-only.
...
It is disabled in the perf application config. Enabling it requires

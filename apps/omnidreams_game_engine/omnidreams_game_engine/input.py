...
class DriverInput:
    ...
    def sample(
        ...
        """Apply the latest retained command from the first frame in ``window``."""
        transitions = sorted(
        ...
        if latest_transition is not None:
            current_command = latest_transition.command
        ...
        sample_count = len(window.sample_times_s)
        commands = [command] * sample_count
        transition_timestamps_us: list[int | None] = [None] * sample_count
        if replayed_transition is not None:
            # ponytail: A completed tap between chunks gets one physics frame.
            # Interruptible generation is the upgrade for true mid-step timing.
            commands[0] = replayed_transition.command
            transition_timestamps_us[0] = replayed_transition.timestamp_us
            if sample_count > 1 and latest_transition is not None:
                transition_timestamps_us[1] = latest_transition.timestamp_us
        elif latest_transition is not None:
            transition_timestamps_us[0] = latest_transition.timestamp_us

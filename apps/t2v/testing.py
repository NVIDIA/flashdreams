...
def check_t2v_model_impl(
    ...
    *,
    ...
    try:
        ...
        if session_desc is None:
            raise ValueError("A T2V application must describe its session.")
...
class _UI:
    """Small retained-widget surface matching what the shared UI constructs."""

    def __init__(self) -> None:
        self.screen = _Widget()

    def Window(self, *args: Any, **kwargs: Any) -> _Widget:
        del args, kwargs
        return _Widget()

    def InputText(
        self, parent: Any, label: str, value: str, callback: Any, **kwargs: Any
    ) -> _Widget:
        del parent, label, callback, kwargs
        return _Widget(value)

    def InputInt(
        self, parent: Any, label: str, value: int, callback: Any, **kwargs: Any
    ) -> _Widget:
        del parent, label, callback, kwargs
        return _Widget(value)

    def Button(self, parent: Any, label: str, callback: Any) -> _Widget:
        del parent, label, callback
        return _Widget()

    def Text(self, parent: Any, value: str) -> _Widget:
        del parent
        return _Widget(value)


class TransparentUIRenderer:
    """Render a transparent CPU overlay while still executing widget code."""
    ...
    def render(self, step_index: int, events: UserInputEvents, draw: Any) -> Tensor:
        draw(self.ui, step_index, events)
        overlay = torch.full((4, self.height, self.width), -1.0)
        overlay[3].zero_()
        return overlay

...
Replaces ``internal/upsampler/benchmark_color_corrector.py`` with a
pytest case marked ``manual`` + ``slow``. For each parametrized
``(dtype, shape)`` row, builds a torch-backend reference and a
cuda-backend candidate corrector, times both with the standard
``cuda.Event`` warmup/iters protocol, asserts ``max_abs`` against a
dtype-appropriate tolerance, and prints the wall time and speedup.

Covers items 8 (rename ``ultraflashvsr_adain_cuda`` ->
``flashvsr_adain_cuda_<csrc_checksum>``, public class rename to
``FlashVSRColorCorrector``) and 9 (the public dispatcher's
``implementation`` knob).
...
GPU launches at the configured shape. Run with::
    ...
    uv run pytest \\
        flashdreams/tests/flashvsr/test_color_corrector_benchmark.py \\
...
from flashdreams.recipes.flashvsr import corrector as cc_module

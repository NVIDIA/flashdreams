...
Replaces the old ``internal/upsampler/benchmark_projector.py`` script;
the contract moves from ``UltraFlashVSRUpsampler.__call__`` to
``FlashVSRPipeline.generate``.

Marked ``manual`` + ``slow`` so it stays opt-in. Run with::

    uv run pytest flashdreams/tests/flashvsr/test_projector_benchmark.py \\
...
from flashdreams.recipes.flashvsr.pipeline import FlashVSRPipeline
from flashdreams.recipes.flashvsr.transformer import FlashVSRTransformer

...
the color corrector (as the AdaIN reference) consume it. See
``README.md`` (sibling) for the streaming chunk contract.
"""
...
from flashdreams.recipes.flashvsr.decoder.network import TAEHV, TAEHV_Cache
...
class FlashVSRDecoderConfig(InstantiateConfig["FlashVSRDecoder"]):
...
class FlashVSRDecoderCache(DecoderAutoregressiveCache):
    ...
    :class:`FlashVSRPipeline.generate` from

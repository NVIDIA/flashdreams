...
The frozen legacy decoder ships under ``flashdreams/tests/flashvsr/_tcdecoder.py``
as a parity reference for ``test_tcdecoder_replacement.py``; this class is
the live implementation and preserves the same FlashVSR-facing API.
"""
...
from flashdreams.recipes.flashvsr.encoder.network import PixelShuffle3d
...
class IdentityConv2d(nn.Conv2d):
    ...
    See ``flashdreams/tests/flashvsr/_tcdecoder.py:IdentityConv2d`` for
    the frozen parity reference; the live class here is structurally
    identical so ``test_tcdecoder_replacement.py`` agrees byte-for-byte.
    """
...
def _apply_identity_deepen(
    ...
    See ``flashdreams/tests/flashvsr/_tcdecoder.py`` for the frozen parity
    reference (the legacy FlashVSR upsampler hard-codes the same deepening).
...
class TAEHV(_FlashDreamsTAEHV):
    ...
    def _post_model_init_hook(self) -> None:
        """Insert FlashVSR-style identity-deepened layers after every ReLU."""
    ...
    def forward(
        ...
        cache: TAEHV_Cache | None = None,
        **kwargs: Any,
    ) -> torch.Tensor:
        """Decode FlashVSR latents, accepting legacy unused kwargs."""
        if cond is not None:

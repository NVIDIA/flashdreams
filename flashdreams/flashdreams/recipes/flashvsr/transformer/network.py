...
inference cache lives in :mod:`flashdreams.recipes.flashvsr.transformer`
(``__init__.py``).

[`flashdreams/tests/flashvsr/_wan_model_dit.py`](../../../tests/flashvsr/_wan_model_dit.py)
is kept verbatim as the frozen parity reference. Numerical parity vs. the
legacy ``WanModel`` was verified bit-for-bit chunk-by-chunk in commit
``3c4affa`` (max_abs = 0 across 4 AR steps).
...
# block-sparse path that distinguishes FlashVSR's DiT from the dense Wan 2.1
# baseline. ``flashdreams/tests/flashvsr/_wan_model_dit.py`` keeps a frozen
# copy for parity testing, but the live import path uses the copies below.
# ---------------------------------------------------------------------------
...
# Self-attention window in (latent_frames, latent_h, latent_w). FlashVSR
# tiny_long fixes this; we keep it as a module-level constant rather than a
# config knob because the topk / local-range math assumes ``win[0]==2`` and
# ``win[1]==win[2]==8``.

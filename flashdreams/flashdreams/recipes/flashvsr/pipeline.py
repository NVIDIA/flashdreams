...
``UltraFlashVSRUpsampler.forward`` exactly. See ``README.md`` (sibling)
...
from flashdreams.recipes.flashvsr.decoder import (
    FlashVSRDecoder,
    FlashVSRDecoderCache,
    FlashVSRDecoderConfig,
)
from flashdreams.recipes.flashvsr.encoder import (
    FlashVSREncoder,
...
class FlashVSRPipelineConfig(StreamInferencePipelineConfig):
    ...
    ``FlashVSRTransformer.initialize_autoregressive_cache(text_embeddings=)``
    on every ``initialize_cache`` call. Set to ``None`` to require callers
    to pass ``prompt_tensor`` to ``initialize_cache`` explicitly."""
...
class FlashVSRPipeline(
    ...
    def __init__(self, config: FlashVSRPipelineConfig) -> None:
        ...
        # Loaded once and reused on every ``initialize_cache`` call. Stays on
        # CPU here; ``initialize_cache`` moves it to ``self.device``. The
        # ``cast`` is honest: ``posi_prompt.pth`` pickles a bare Tensor, not
        # a state dict, so the helper's nominal ``dict[str, Tensor]`` return
        # is wrong for this one file.
        self._prompt_tensor: Tensor | None = (
            cast(Tensor, load_checkpoint(config.prompt_path))

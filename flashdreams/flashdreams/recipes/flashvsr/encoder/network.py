...
(``flashdreams.recipes.flashvsr.decoder.network``) to pack the bicubic
conditioning into latent channels; the decoder needs the
``autopad_first_frame=True`` + ``out_layout="frame_first"`` mode while
the projector needs the default ``out_layout="channel_first"`` mode.
"""
...
# FlashVSR's projector pads with replicate vs the Wan VAE's zero-pad;
# the ``pad_mode`` knob is first-class on the shared ``CausalConv3d``,
# so a partial keeps the call sites below unchanged. RMS_norm is reused
# directly: the projector's calls all use ``bias=False`` (the default),
# matching the Wan VAE behavior byte-for-byte.

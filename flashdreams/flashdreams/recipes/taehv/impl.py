...
class TAEHV(nn.Module):
    ...
    # Class-level annotations so ``self.decoder`` and ``self._decoder_call``
    # narrow to their actual concrete types instead of going through
    # ``nn.Module.__getattr__`` (which returns ``Tensor | Module``).
    # ``_decoder_call`` is set via ``object.__setattr__`` in ``__init__`` to
    # dodge ``nn.Module.__setattr__`` re-registration; without this
    # annotation ty cannot see its callable type at the use site.
    decoder: "Decoder"
    ...
    def __init__(
        ...
        # Subclasses can mutate the module tree here (e.g., insert deepened
        # layers) before any checkpoint is loaded.
        ...
        # The state-dict pre-hook (legacy key remap + TGrow patch) runs
        # inside ``self.load_state_dict`` below.

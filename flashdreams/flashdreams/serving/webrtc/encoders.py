...
Two implementations share a single ``VideoEncoder`` protocol:

- :class:`DefaultRTCEncoder` — thin adapter over aiortc's built-in software
  encoder path. Frames leave the model as raw RGB and aiortc's RTP sender
  loop drives encoding.
- :class:`PyNvHardwareEncoder` — GPU-resident NVENC H.264 via
  ``PyNvVideoCodec`` 2.1. Emits pre-encoded Annex-B packets that aiortc's
  ``H264Encoder.pack()`` fragments into RTP payloads (no re-encoding).

Selection is performed by :func:`select_encoder`, which layers a
capability query (``GetEncoderCaps``) with two failure semantics: an
environmental "no" from Stage 1 falls back silently to the software path,
while a Stage-2 ``CreateEncoder`` failure is logged with traceback and
re-raised so it cannot be masked by a silent fallback.
"""
...
class VideoEncoder(Protocol):
    ...
    """SDP codec preference hint. ``"h264"`` for the NVENC path (must be
    ...
    async def deliver_chunk(
        ...
        *,
        ...
        """Encode ``chunk`` and enqueue the resulting frames/packets."""
        ...

    def close(self) -> None:
        """Release any encoder-owned resources (hardware handles, etc.)."""
        ...


# ---------------------------------------------------------------------------
# Software path (aiortc's built-in encoder)
# ---------------------------------------------------------------------------


class DefaultRTCEncoder:
    """Software path: hand raw RGB frames to aiortc and let it encode.

    This class does not encode itself; it wraps a
    :class:`~flashdreams.serving.webrtc.media.BufferedVideoTrack` that
    carries raw RGB frames, and aiortc's RTP sender loop drives encoding
    frame-by-frame. The concrete codec (VP8, VP9, H.264) is chosen by SDP
    negotiation.
    """
    ...
    async def deliver_chunk(
        ...
        *,
        ...
        # aiortc's software encoder chooses keyframe cadence internally
        # and responds to receiver PLI/FIR feedback, so a caller-supplied
        # force_keyframe is neither honoured nor useful on this path.
        del force_keyframe
        from flashdreams.serving.webrtc.media import BufferedVideoTrack
...
# ---------------------------------------------------------------------------
# Hardware path (NVENC via PyNvVideoCodec 2.1)
# ---------------------------------------------------------------------------
...
class PyNvHardwareEncoder:
    ...
    def __init__(
        ...
        *,
        ...
        if not _PYNVVIDEOCODEC_AVAILABLE:
            raise RuntimeError(
                "PyNvVideoCodec is not installed; PyNvHardwareEncoder cannot "
                "be used. Install with `pip install PyNvVideoCodec` or "
        ...
        self._encoder = nvc.CreateEncoder(  # type: ignore[attr-defined]
            width=width,
            height=height,
            fmt="ABGR",
    ...
    def create_track(self, *, maxsize: int) -> NVENCVideoTrack:
        # Lazy import to avoid an ``encoders`` ↔ ``media`` cycle.
...
def select_encoder(
    *,
    ...
    not touch the NVENC probe path at all.
    ...
    * **Stage 1** (``GetEncoderCaps`` + resolution bounds): a "no" here
      means the environment cannot do this, which is expected on hosts
      without NVENC. Fall back silently to
      :class:`DefaultRTCEncoder`, logging the reason at INFO.
    * **Stage 2** (``CreateEncoder``): a raise here means Stage 1 said
    ...
    if not supported:
        ...
        logger.info(
            "NVENC H.264 not supported on this system ({}); using aiortc "
            "software encoder.",
            reason,
        )
        return DefaultRTCEncoder(fps=fps)

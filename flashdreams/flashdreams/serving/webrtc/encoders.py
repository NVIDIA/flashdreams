...
def _pynvvideocodec_installed() -> bool:
    """Return whether ``PyNvVideoCodec`` is installed, checked *without* importing.

    ``importlib.util.find_spec`` walks the import machinery to find the
    package on ``sys.path`` but does not execute its ``__init__.py``, so
    it has no side effects on the calling process's CUDA / shared-library
    state. Callers that go on to actually use NVENC (only
    :func:`select_encoder` today) trigger the real import via
...
def select_encoder(
    ...
    * **Stage 1** (``GetEncoderCaps`` + resolution bounds): a "no" here
      ...
      without NVENC. Fall back silently to :class:`DefaultRTCEncoder`,
      logging the reason at INFO.

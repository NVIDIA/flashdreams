...
"""Cheap import-time checks for the helios plugin."""
...
from helios.config import RUNNER_CONFIGS
...
def test_entry_points_match_module_literals() -> None:
    ...
    for slug, target in entries.items():
        ...
        assert module_name == "helios.config"
...
def test_entry_points_discoverable_when_installed() -> None:
    ...
    discovered = {ep.name for ep in eps if ep.value.startswith("helios.")}
    if not discovered:
        pytest.skip(
            "plugin not installed; run `uv sync --project integrations/helios` first"
        )
    assert discovered == set(RUNNER_CONFIGS)


def test_chunk_frames_constant() -> None:
    from helios.pipeline import HELIOS_CHUNK_FRAMES

    assert HELIOS_CHUNK_FRAMES == 33

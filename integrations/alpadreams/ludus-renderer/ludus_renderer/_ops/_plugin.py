...
def _get_plugin(gl=True):
    ...
    Args:
        gl: Must be True for Ludus renderer (mesh shaders require OpenGL).
            Kept as a parameter for backward-compat with call sites that
            still pass it explicitly; gl=False raises.
    ...
    # Ludus renderer only supports GL mode (mesh shaders require OpenGL)
    if not gl:
        raise RuntimeError("Ludus renderer requires gl=True (mesh shaders need OpenGL)")
    
    # Source files - paths relative to this file's location
    # This file is in ludus_renderer/_ops/
    # C++ files are in ludus_renderer/_cpp/
    source_files = [
    ...
    plugin_name = 'ludus_renderer_plugin' + ('_gl' if gl else '')
    try:
        lock_fn = os.path.join(torch.utils.cpp_extension._get_build_directory(plugin_name, False), 'lock')
        if os.path.exists(lock_fn):
            logging.getLogger('ludus_renderer').warning("Lock file exists in build directory: '%s'" % lock_fn)
    except:
        pass

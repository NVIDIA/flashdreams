...
"""Cheap import-time checks for the ``alpadreams`` plugin.
...
``pipeline.recipe_name``, descriptions are non-empty, and the
entry-point declarations in ``pyproject.toml`` match the
``alpadreams.config`` ``RUNNER_*`` literals exactly.
"""
...
import pytest
import tomllib
from alpadreams import config as config_mod
from alpadreams.config import ALPADREAMS_RUNNERS
...
def test_runners_dict_is_non_empty() -> None:
    ...
    assert ALPADREAMS_RUNNERS, "ALPADREAMS_RUNNERS is empty"
...
def test_runner_name_mirrors_pipeline_recipe_name() -> None:
    ...
    drifted = {
        ...
        for slug, cfg in ALPADREAMS_RUNNERS.items()
...
def test_runners_have_descriptions() -> None:
    ...
    empty = [
        slug for slug, cfg in ALPADREAMS_RUNNERS.items() if not cfg.description.strip()
    ]
...
def test_entry_points_match_module_literals() -> None:
    ...
    module_slugs = set(ALPADREAMS_RUNNERS)
    ...
    for slug, target in entries.items():
        ...
        assert module_name == "alpadreams.config", (
...
def test_entry_points_discoverable_when_installed() -> None:
    ...
    discovered = {ep.name for ep in eps if ep.value.startswith("alpadreams.")}
    if not discovered:
        pytest.skip("plugin not installed; run `uv sync` from the repo root first")
    assert discovered == set(ALPADREAMS_RUNNERS), (
        f"discovered slugs ({sorted(discovered)}) != "
        f"plugin runners ({sorted(ALPADREAMS_RUNNERS)})"
    )

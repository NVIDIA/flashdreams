...
"""Cheap import-time checks for the ``self_forcing`` plugin."""
...
def test_entry_points_match_module_literals() -> None:
    ...
    for slug, target in entries.items():
        ...
        assert module_name == "self_forcing.config", (
...
def test_entry_points_discoverable_when_installed() -> None:
    ...
    discovered = {ep.name for ep in eps if ep.value.startswith("self_forcing.")}
    if not discovered:

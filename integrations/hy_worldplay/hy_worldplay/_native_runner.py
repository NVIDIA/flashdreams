...
class HyWorldPlayWanI2VNativeRunner(Runner["HyWorldPlayWanI2VRunnerConfig", WanInferencePipeline]):
    ...
    Action conditioning, camera-trajectory PRoPE, distilled-checkpoint
    weight remap, and reconstituted-context memory (selection +
    KV-prefill executor) activate when the matching flags on
    :class:`HyWorldPlayWanI2VRunnerConfig` are set. The encoder /
    transformer / network swap is wired in the config's
    ``__post_init__``; this runner only binds the per-rollout payloads
    (action labels, viewmats + intrinsics, memory-selection knobs) on
    the encoder before the AR loop starts.
    """
    ...
    def run(self) -> None:
        ...
        # ``HY_VENDOR_NOISE_MODE=1`` swaps the per-chunk private RNG
        # draw for a single big ``randn`` pre-drawn with the rollout
        # seed and sliced per AR step, matching vendor's
        # ``prepare_latents`` so native and vendor consume bit-identical
        # noise. See :meth:`_maybe_vendor_aligned_noise_ctx`.
        ...
        if cfg.use_memory_selection:
            ...
            # encoder is guaranteed to have its viewmats bound here.
    ...
    def _bind_camera_data(self) -> None:
        ...
        so callers can flip the action and camera flags independently.
        """
        from hy_worldplay._action import HyWorldPlayWanCtrlEncoder
    ...
    def _resolve_encoder_and_n_latents(
        ...
        """Return ``(encoder, n_latents)`` after asserting the encoder swap ran.

        ``flag_name`` is woven into the assertion text so misconfigured

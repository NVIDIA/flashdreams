...
# flashdreams-lingbot2

LingBot-World v2 streaming camera-control I2V integration + a minimal WebRTC
...
| slug | description |
| --- | --- |
| `lingbot-world-v2-14b-causal-fast` | LingBot-World v2 14B causal-fast streaming camera-control I2V (Wan VAE decoder, 4-step). |
| `lingbot-world-v2-14b-causal-fast-taehv-window15-sink3` | LightTAE decoder swap with `window_size_t=15` + `sink_size_t=3` for tighter interactive streaming. |
...
```bash
uv pip install -e integrations/lingbot2
```
...
```bash
# List every registered runner (this plugin's slugs appear under "lingbot-world-v2-*").
uv run flashdreams-run --help

# Per-runner help: every overridable field is a CLI flag.
uv run flashdreams-run lingbot-world-v2-14b-causal-fast --help

# Single-GPU demo with the bundled example assets (lazy-downloaded
# from the upstream LingBot-World v2 GitHub examples folder on first run).
uv run flashdreams-run lingbot-world-v2-14b-causal-fast --example-data True --total-blocks 21
...
uv run torchrun --nproc_per_node=4 --no-python flashdreams-run \
    lingbot-world-v2-14b-causal-fast --example-data True --total-blocks 21
```
...
Access via runner.
```python
from lingbot2.config import RUNNER_LINGBOT_WORLD_V2_14B_CAUSAL_FAST as runner_config
...
```python
import torch
from lingbot2.config import PIPELINE_LINGBOT_WORLD_V2_14B_CAUSAL_FAST as pipeline_config
from lingbot2.encoder.camctrl import CamCtrlInput
...
The `lingbot2.webrtc` subpackage exposes a minimal WebRTC server that
...
- Action-bound control flow:
  ...
  2. server runs one Lingbot2 AR inference chunk,
  3. server enqueues chunk frames to the WebRTC track and emits `chunk_done`.

From repository root:

```bash
uv run --package flashdreams-lingbot2 python -m lingbot2.webrtc.server \
    --host 0.0.0.0 --port 8089 --config_name lingbot-world-v2-14b-causal-fast-taehv-window15-sink3

# 4 GPUs
uv run --package flashdreams-lingbot2 \
  python -m torch.distributed.run --standalone --nnodes=1 --nproc_per_node=4 \
  -m lingbot2.webrtc.server \
...
- CUDA-capable GPU.
- `HF_TOKEN` exported. The `robbyant/lingbot-world-v2-14b-causal-fast`
  checkpoint
...
- Example assets (`image.jpg`, `intrinsics.npy`, `poses.npy`,
  ...
  examples folder into `assets/example_data/lingbot_world_v2/<NN>/` on
...
Text events use the same DataChannel and are optional for other models:
...
- Supported `state` values:
  - `trigger` / `hold` / `on`: activate an advertised event.
  - `clear` / `release` / `off` / `none`: restore the base prompt context.
- The initial-scene payload includes `capabilities.text_events`,
  `event_catalog`, and `active_event_id`. Browsers should only render event
  controls when `capabilities.text_events` is true.
- Servers acknowledge with `event_ack` and include the current
  `active_event_id`.
...
```bash
uv run --extra dev pytest integrations/lingbot2/tests
```

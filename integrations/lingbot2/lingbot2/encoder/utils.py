...
## Lingbot2 World example-data preprocessing

_TEMPORAL_COMPRESSION_RATIO = 4
"""Lingbot2 World VAE temporal stride; one encoded frame per N video frames."""
...
def preprocess_example_poses(poses: np.ndarray) -> tuple[np.ndarray, float]:
    """Preprocess the example poses carried over from the original Lingbot2 World repo.
    ...
    # Match upstream Lingbot2 World: world-scale normalizer is computed
...
def interpolate_camera_poses(
    ...
    try:
        ...
        raise ImportError(
            ...
            "`pip install scipy` (or `pip install flashdreams-lingbot2`)."
        ) from e

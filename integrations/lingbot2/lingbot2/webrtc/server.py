...
def _get_lingbot2_manager(app: web.Application) -> Lingbot2SessionManager:
    return cast(Lingbot2SessionManager, app[SESSION_MANAGER_KEY])


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Lingbot2 WebRTC server: serves /request_session and streams action-bound "
    ...
    parser.add_argument(
        "--config_name",
        type=str,
        default="lingbot-world-v2-14b-causal-fast",
        help="LingBot-World v2 config preset from PIPELINE_CONFIGS.",
    )
    parser.add_argument(
        "--no_compile",
        action="store_true",
        help="Disable torch.compile when building the Lingbot2 pipeline.",
    )
    parser.add_argument(
        "--device",
        type=str,
        default="cuda:0",
        help="Torch device used for the Lingbot2 runtime.",
    )
    parser.add_argument(
        "--warmup_chunks",
        type=int,
    ...
    parser.add_argument(
        ...
        help="Example folder index under assets/example_data/lingbot_world_v2.",
    )
...
def create_app(
    *,
    ...
    manager = session_manager or Lingbot2WebRTCSessionManager()
    app = create_webrtc_app(
        web_dir=WEB_DIR,
        session_manager=manager,
        preload_name="Lingbot2",
...
def initialize_distributed(
    *, default_device: str | torch.device = "cuda:0"
    ...
    if not torch.cuda.is_available():
        raise RuntimeError(
            "CUDA is required for inference in the Lingbot2 WebRTC server."
        )
    ...
    logger.info(
        "Rank {} initialized Lingbot2 runtime with context_parallel_size {}",
        world_rank,
        world_size,
    )
...
def main() -> None:
    ...
    session_manager = Lingbot2WebRTCSessionManager(

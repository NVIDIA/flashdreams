...
"""Lingbot2 World DiT network: Wan 2.1 backbone with per-block camera control."""
...
@dataclass
class Lingbot2WorldDiTNetworkCache(WanDiTNetworkCache):
    """Cache container for all transformer blocks."""


@dataclass
class Lingbot2WorldDiTNetworkConfig(WanDiTNetworkConfig):
    """Wan-sized hyperparameters plus Lingbot2 camera / action control."""

    _target: type["Lingbot2WorldDiTNetwork"] = field(
        default_factory=lambda: Lingbot2WorldDiTNetwork
    )
    control_type: Literal["cam", "act"] = "cam"


@dataclass
class Lingbot2WorldDiTNetwork1pt3BConfig(Lingbot2WorldDiTNetworkConfig):
    """Configuration for the 1.3B Lingbot2 World DiT network."""
...
@dataclass
class Lingbot2WorldDiTNetwork14BConfig(Lingbot2WorldDiTNetworkConfig):
    """Configuration for the 14B Lingbot2 World DiT network."""
...
class Lingbot2WorldDiTNetwork(WanDiTNetwork):
    """Lingbot2 World DiT diffusion backbone for text-to-video and image-to-video."""

    def __init__(self, config: Lingbot2WorldDiTNetworkConfig) -> None:
    ...
    def replace_text_embeddings(
        self,
        cache: Lingbot2WorldDiTNetworkCache,
        text_embeddings: Tensor,
    ) -> None:
        """Replace cached cross-attention text K/V for all blocks.

        LingBot-World v2 text events are represented as alternate UMT5
        embeddings. The self-attention KV cache stays intact so the
        rollout horizon is preserved; only the static text context used
        by cross-attention is swapped before the next chunk.
        """

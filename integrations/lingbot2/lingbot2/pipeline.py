...
"""Lingbot2 World streaming inference pipeline (camera-controlled I2V)."""
...
class Lingbot2UMT5TextEncoderConfig(UMT5TextEncoderConfig):
...
@dataclass(kw_only=True)
class Lingbot2WorldInferencePipelineConfig(WanInferencePipelineConfig):
    """Config for the Lingbot2 World streaming pipeline.

    Same shape as the Wan I2V config; only the target class is overridden.
    """

    _target: type["Lingbot2WorldInferencePipeline"] = field(
        default_factory=lambda: Lingbot2WorldInferencePipeline
    )
    text_encoder: Lingbot2UMT5TextEncoderConfig | None = field(
        default_factory=Lingbot2UMT5TextEncoderConfig
    )


class Lingbot2WorldInferencePipeline(WanInferencePipeline):
    """Streaming camera-controlled I2V pipeline for Lingbot2 World.
    ...
    def generate(
        ...
        Args:
            ...
            cache: Cache from ``initialize_cache``; ``cache.image`` must
                be populated (Lingbot2 World is I2V-only).
        ...
        assert cache.image is not None, (
            "Lingbot2WorldInferencePipeline is I2V-only; pass ``image=...`` "
            "to ``initialize_cache``."
        )

...
"""LingBot-World v2 camera-control I2V runner classes."""
...
from lingbot2.encoder.utils import (
    get_Ks_transformed,
    preprocess_example_poses,
)
from lingbot2.pipeline import (
    Lingbot2WorldInferencePipeline,
)

__all__ = [
    "Lingbot2WorldRunnerConfig",
    "Lingbot2WorldRunner",
]
...
# ``lingbot2/runner.py`` -> ``lingbot2/`` -> ``integrations/lingbot2/`` ->
...
EXAMPLE_DATA_DIR_LOCAL = _REPO_ROOT / "assets/example_data/lingbot_world_v2"
...
"""Example files required by :class:`Lingbot2WorldRunner`'s demo mode."""
...
def ensure_example_data_downloaded(*, is_rank_zero: bool, example_idx: int) -> Path:
    ...
    The runner calls this from :meth:`Lingbot2WorldRunner._fill_example_data_defaults`;
    the WebRTC server calls it from its ``main()`` so the same files
    land on disk before the server's
    ``Lingbot2WebRTCSessionManager._initialize_sync`` checks for them. The
    download itself is small (image + intrinsics + poses + prompt for
    the chosen example), uses the public LingBot-World v2 GitHub raw URLs,
...
class Lingbot2WorldRunnerConfig(RunnerConfig):
    """Runner config for every shipped LingBot-World v2 variant."""

    _target: type["Lingbot2WorldRunner"] = field(
        default_factory=lambda: Lingbot2WorldRunner
    )
    ...
    fps: int = 16
    """Output video frame rate. LingBot-World v2 was trained at 16fps."""

    example_data: bool = False
    """When ``True``, lazy-download bundled GitHub example assets into
    ``assets/example_data/lingbot_world_v2/`` and fill ``image_path`` /
...
class Lingbot2WorldRunner(
    Runner[Lingbot2WorldRunnerConfig, Lingbot2WorldInferencePipeline]
):
    """Streaming camera-control I2V driver."""

    config: Lingbot2WorldRunnerConfig
    ...
    def run(self) -> None:
        ...
        assert cfg.image_path is not None, (
            "Lingbot2WorldRunner requires --image_path (first-frame RGB image)."
        )
        assert cfg.pose_path is not None, (
            "Lingbot2WorldRunner requires --pose_path "
            "(.npy of [T, 4, 4] camera-to-world matrices)."
        )
        assert cfg.intrinsic_path is not None, (
            "Lingbot2WorldRunner requires --intrinsic_path "
...
## I/O helpers (``cv2`` / ``mediapy`` are listed under ``flashdreams-lingbot2``'s
...
def _load_first_frame(
    ...
    try:
        ...
        raise ImportError(
            ...
            "Install the lingbot2 plugin: pip install flashdreams-lingbot2."
        ) from exc

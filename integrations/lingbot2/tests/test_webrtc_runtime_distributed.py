...
def test_initialize_sync_passes_rank_seed(
    ...
    derive_calls: list[dict[str, Any]] = []
    pipeline_events: list[tuple[Any, ...]] = []
    _patch_pipeline_factory(monkeypatch, "TestLingbot2", derive_calls, pipeline_events)

    runtime = session.Lingbot2InferenceRuntime(
        config=session.Lingbot2RuntimeConfig(
            config_name="TestLingbot2",
...
def test_reset_rollout_accepts_uploaded_prompt_and_first_frame(
    ...
    runtime._reset_rollout_sync(
        session_input=session.Lingbot2SessionInput(
...
def test_runtime_distributed_ops_use_world_cp_and_rank_seed(
    ...
    if not torch.cuda.is_available():
        pytest.skip("Distributed Lingbot2 runtime test requires CUDA.")
    ...
    _patch_pipeline_factory(
        monkeypatch, "DistributedTestLingbot2", derive_calls, pipeline_events
    )

    runtime = session.Lingbot2InferenceRuntime(
        config=session.Lingbot2RuntimeConfig(
            config_name="DistributedTestLingbot2",
            seed=100,

...
"""Cheap import-time checks for the ``lingbot2`` plugin."""
...
from lingbot2 import config as config_mod
from lingbot2.config import RUNNER_CONFIGS
from lingbot2.transformer import (
    LINGBOT_WORLD_MIN_CHECKPOINT_FREE_GB,
    Lingbot2WorldTransformerConfig,
)
...
def test_lingbot2_configs_carry_documented_checkpoint_disk_requirement() -> None:
    """LingBot2's large checkpoint should preflight its documented first-run budget."""
    for cfg in RUNNER_CONFIGS.values():
        transformer = cfg.pipeline.diffusion_model.transformer
        assert isinstance(transformer, Lingbot2WorldTransformerConfig)
        assert (
...
def test_entry_points_match_module_literals() -> None:
    ...
    for slug, target in entries.items():
        ...
        assert module_name == "lingbot2.config", (
...
def test_entry_points_discoverable_when_installed() -> None:
    ...
    discovered = {ep.name for ep in eps if ep.value.startswith("lingbot2.")}
    if not discovered:

...
# Sample root, derived from this file's location so all packaged-asset defaults
# below resolve correctly regardless of the user's cwd. Layout:
#   ``<sample-root>/src/interactive_drive/cli.py`` (this file)
#   ``<sample-root>/assets/scenes/``
#   ``<sample-root>/configs/{example_world_model.yaml,wheels/}``
# This matches the flashdreams convention of invoking everything from the
# workspace root via ``uv run --package interactive-drive ...`` — users don't
# have to ``cd`` anywhere for the defaults to land on the bundled assets.
_SAMPLE_ROOT = Path(__file__).resolve().parent.parent.parent

# Default scene staged by ``prepare.py`` from ``nvidia/omni-dreams-scenes``.
DEFAULT_SCENE = _SAMPLE_ROOT / "assets/scenes/clipgt-01d503d4-449b-46fc-8d78-9085e70d3554.usdz"

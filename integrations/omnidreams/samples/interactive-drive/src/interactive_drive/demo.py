...
def build_parser() -> argparse.ArgumentParser:
    ...
    * The ``--no-hud`` toggle itself, plus ``--port`` (the supervisor
    ...
    parser.set_defaults(
        backend="omnidreams",
        manifest=_cli._SAMPLE_ROOT / "configs/example_world_model.yaml",
    )
    parser.description = (
    ...
    parser.add_argument(
        "--scene-dir",
        type=Path,
        default=_cli._SAMPLE_ROOT / "assets/scenes",
    ...
    parser.add_argument(
        "--wheel-profiles-dir", type=Path, default=_cli._SAMPLE_ROOT / "configs/wheels"
    )
    parser.add_argument(
        "--control-assets-dir",
        type=Path,
...
def _maybe_autostage_scene(scene: Path) -> Path:
    ...
    if scene.exists():
        return scene
    bundled_dir = (_cli._SAMPLE_ROOT / "assets" / "scenes").resolve()
    ...
    from interactive_drive.prepare import stage_scene

    return stage_scene(_cli._SAMPLE_ROOT, stem, force=False)

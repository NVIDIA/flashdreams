...
void main() {
    ...
    uint pool_id_local = temp % pc.u_num_polyline_pools;
    uint query_id_local = temp / pc.u_num_polyline_pools;
    ...
    if (pool_id_local >= scene.num_polyline_pools) {
        _task_count = 0u;
        EmitMeshTasksEXT(_task_count, 1, 1); return;
    }
    ...
    if (varray_offset >= num_varrays) {
        // Keep subsequent SSBO reads in-bounds, then force the final emit
        // count to zero. This mirrors the polygon task shader guard and
        // prevents over-dispatched workgroups for smaller pools from
        // reading following pools' prefix sums/vertices.
    ...
    payload.total_points = total_pts;
    payload.width = get_prim_width(pool.prim_type_id, query.camera_type_id);
    payload.color = get_prim_color(pool.prim_type_id);
    payload.cap_style = CAP_ROUND;

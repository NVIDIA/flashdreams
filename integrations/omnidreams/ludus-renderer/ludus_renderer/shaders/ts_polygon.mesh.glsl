...
void main() {
    ...
    if (payload.varray_idx > 0u) {
        tri_start = uint(g_int32[scene.int32_buffer_offset + pool.tri_ps_offset + payload.varray_idx - 1u]);
    }
    ...
    if (subdiv == 0u && payload.vertex_count <= MAX_SHARED_VERTS && chunk_id == 0u) {
        ...
        if (tid < num_orig_tris) {
            ...
            for (uint t = 0u; t < tris_per_tri; t++) {
                ...
                gl_MeshPrimitivesEXT[tri_base + t].gl_Layer = int(payload.query_id);
                prim_out[tri_base + t].color = payload.color;
                prim_out[tri_base + t].is_bev = payload.is_bev;
            }
        }
    } else if (subdiv == 2u) {
        ...
        if (tri_local < num_orig_tris && vert_local < 16u) {
            ...
            prim_out[tri_out_idx].color = payload.color;
            prim_out[tri_out_idx].is_bev = payload.is_bev;
        }
    } else if (subdiv == 3u) {
        ...
        for (uint t = tid; t < 64u; t += 32u) {
            ...
            uint idx_base = t * 3u;
            gl_PrimitiveTriangleIndicesEXT[(idx_base) / 3u] = uvec3(idx.x, idx.y, idx.z);
            gl_MeshPrimitivesEXT[t].gl_Layer = int(payload.query_id);
            prim_out[t].color = payload.color;
            prim_out[t].is_bev = payload.is_bev;
        }
    } else {

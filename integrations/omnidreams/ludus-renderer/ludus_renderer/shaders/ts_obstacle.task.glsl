...
void main() {
    ...
    // count to zero. Without this, invalid workgroups can read the next
    // pool's prefix sums / translations / scales and produce ghost cubes
    // scattered throughout the scene.
    bool force_zero_tasks = false;
    if (obstacle_id_local >= pool.num_cubes) {
        force_zero_tasks = true;
        obstacle_id_local = 0u;
    }
    ...
    payload.scale = scale_local;
    payload.front_color = front;
    payload.back_color = back;
    payload.subdivision_level = max_subdiv;
    payload.render_flags = pool.render_flags;
    payload.is_bev = (query.camera_type_id == CAMERA_TYPE_BEV) ? 1.0 : 0.0;
    payload.face_mask = fmask;

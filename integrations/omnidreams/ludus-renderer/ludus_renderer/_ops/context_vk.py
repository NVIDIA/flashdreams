...
- Rendering preserves the CUDA backend's one-batched-call scheduling model;
  Vulkan records one task/mesh/fragment submission for the full query batch.
- Geometry is generated procedurally in task/mesh shaders rather than in
  CUDA kernels.
- Fixed-function color/depth rendering resolves visibility, then compute writes
  a CUDA-imported output SSBO exposed as the returned tensor without an interop
  copy or layered CUDA image import.
...
class LudusTimestampedContext:
    ...
    def __init__(self, device=None):
        ...
        # Keep the default CUDA context's import path free of Vulkan extension
        # discovery and cpp_extension setup.
        ...
        # Mirror the CUDA context's scene bookkeeping so mutable cube pools can
        # update only their shared-memory slices without rebuilding static map
        # buffers. ``scene`` is retained only for rare topology replacement.
    ...
    def upload_scene(self, scene: TimestampedScene) -> int:
            ...
            cube_pool_metadata.append(
                {
                    ...
                    "n_track_poses": n_track,
                    "absolute_pool_index": self._global_cb_pool_off
                    + len(cube_pool_rows)
                    - 1,
                    "timestamp_offset": self._global_ts_off + ts_off,
                    "track_timestamp_offset": self._global_ts_off
                    + ts_off
                    + n_global_ts,
                    "int32_offset": self._global_i32_off + i32_off,
                    "translation_offset": self._global_float_off + float_off,
                    "quaternion_offset": self._global_float_off
                    + float_off
                    + n_track * 3,
                    "scale_offset": self._global_float_off + float_off + n_track * 7,
                    "color_offset": self._global_float_off
                    + float_off
                    + n_track * 7
                    + n_cubes * 3,
                }
        ...
        if scene_id != len(self._scenes):
            raise RuntimeError(
                "Vulkan scene ids must remain dense and append-only during upload"
            )
        self._scenes.append(
            {
                "scene": scene,
                "cube_pool_metadata": cube_pool_metadata,
                "cube_pool_rows": cube_pool_rows,
            }
        )
        return scene_id
    ...
    def remove_scene(self, scene_id: int) -> None:
        ...
        self._scenes[scene_id]["active"] = False
    ...
    def replace_scene(self, scene_id: int, scene: TimestampedScene) -> int:
        """Replace a scene while preserving its externally visible id.

        Topology changes are intentionally the slow control path. Repacking all
        active scene descriptors keeps Vulkan's append-only SSBO layout simple;
        same-topology object motion uses :meth:`update_cube_pool_at_index` and
        touches only the mutable shared-memory slices.
        """
        if not 0 <= scene_id < len(self._scenes):
            raise IndexError(f"scene_id {scene_id} is out of range")
        scenes = [entry["scene"] for entry in self._scenes]
        scenes[scene_id] = scene
        self.clear_scenes()
        for expected_id, replacement in enumerate(scenes):
            uploaded_id = self.upload_scene(replacement)
            if uploaded_id != expected_id:
                raise RuntimeError("Vulkan scene ids changed during replacement")
        return scene_id

    def update_cube_pool(
        self, scene_id: int, prim_type_id: int, pool: CubePool
    ) -> bool:
        """Update the unique same-topology cube pool of a primitive type."""
        if not 0 <= scene_id < len(self._scenes):
            raise IndexError(f"scene_id {scene_id} is out of range")
        candidates = [
            index
            for index, metadata in enumerate(
                self._scenes[scene_id]["cube_pool_metadata"]
            )
            if metadata["prim_type_id"] == int(prim_type_id)
        ]
        if len(candidates) != 1:
            return False
        return self.update_cube_pool_at_index(scene_id, candidates[0], pool)

    def update_cube_pool_at_index(
        self, scene_id: int, pool_index: int, pool: CubePool
    ) -> bool:
        """Update one cube pool in-place when its allocation shape is stable."""
        if not 0 <= scene_id < len(self._scenes):
            raise IndexError(f"scene_id {scene_id} is out of range")
        entry = self._scenes[scene_id]
        metadata_values = entry["cube_pool_metadata"]
        ...
        def _gpu(tensor: torch.Tensor, dtype: torch.dtype) -> torch.Tensor:
            return tensor.to(
                device=self._device, dtype=dtype, non_blocking=True
            ).contiguous()

        # Pool offsets remain local to the scene descriptor. Only render flags
        # are mutable, so preserve the packed header and patch that field.
        pool_row = entry["cube_pool_rows"][pool_index].clone()
        pool_row[11] = int(pool.render_flags)
        pool_header = pool_row.to(self._device).contiguous().view(torch.uint8)

        self.cpp_wrapper.update_cube_pool(
            metadata["absolute_pool_index"],
            ...
            _gpu(pool.timestamps_us, torch.int64),
            _gpu(pool.track_timestamps_us, torch.int64),
            _gpu(pool.cube_ts_prefix_sum, torch.int32),
            _gpu(pool.translations, torch.float32),
            _gpu(pool.quaternions, torch.float32),
            _gpu(pool.scales, torch.float32),
            _gpu(pool.colors, torch.float32),
            pool_header,
        )
        entry["cube_pool_rows"][pool_index] = pool_row
        entry["scene"].cube_pools[pool_index] = pool
        return True
    ...
    def render_batch(
        ...
        scene_ids = torch.tensor([int(q[0]) for q in queries], dtype=torch.int32)
        camera_ids = torch.tensor([int(q[1]) for q in queries], dtype=torch.int32)
        timestamps_us = torch.tensor([_ts(q) for q in queries], dtype=torch.int64)
        camera_type_ids = torch.tensor(
    ...
    def render_uniform(
        ...
        *,
        ...
        """Render a uniform batch through the same single-submit fast path."""
        if self._camera_intrinsics is None:
            raise RuntimeError("call upload_cameras() before render_uniform()")
        if not 0 <= scene_id < len(self._scenes):
            raise IndexError(f"scene_id {scene_id} is out of range")
        ...
        timestamp_tensor = torch.as_tensor(timestamps_us, dtype=torch.int64).cpu()
        return self.render(
            torch.full((frame_count,), scene_id, dtype=torch.int32),
            torch.full((frame_count,), camera_id, dtype=torch.int32),
            timestamp_tensor,
            torch.full((frame_count,), camera_type_id, dtype=torch.int32),
            camera_poses,
            resolution,
        )


# Explicit name for backend selectors; retain the shorter historical name for
# callers that imported the prototype directly.
LudusVulkanTimestampedContext = LudusTimestampedContext

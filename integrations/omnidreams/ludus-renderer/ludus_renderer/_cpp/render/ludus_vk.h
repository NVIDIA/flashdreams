...
constexpr int LUDUS_VK_OUTPUT_SLOTS = 8;
constexpr int LUDUS_VK_RENDER_TARGET_CACHE_SIZE = 4;

struct LudusVkRenderTarget
{
    int                     width;
    int                     height;
    int                     maxLayers;
...
struct LudusTimestampedVkState
{
    ...
    // ---------- Color/depth render pass and reusable framebuffer cache ----------
    VkRenderPass            renderPass;
    LudusVkRenderTarget     renderTargets[LUDUS_VK_RENDER_TARGET_CACHE_SIZE];
    int                     numRenderTargets;
    int                     activeRenderTarget;
    ...
    VkDescriptorSet         descriptorSets[VkContext::kFramesInFlight];
    ...
    VkPipeline              pipelineExport;

    // ---------- Shader modules: 3 graphics pipelines + one compute export ----------
    VkShaderModule          shaderModules[10];
    ...
    // CUDA maps the shared output directly. The next render's inputs-ready
    // signal also releases a reused output slot from CUDA to Vulkan.
    uint64_t                lastVulkanDoneValue;
};
...
struct LudusPushConstants
{
    ...
    uint32_t u_output_width;               // 84
    uint32_t u_output_height;              // 88
};
static_assert(sizeof(LudusPushConstants) == 92, "LudusPushConstants must be 92 bytes");

...
def test_encode_chunk_produces_annex_b_keyframe_with_sps_pps(
    ...
    encoder = PyNvHardwareEncoder(
        width=512, height=288, fps=30, bitrate=2_000_000, gpu_id=0, gop=15,
    )
    try:
        # 4-frame CUDA chunk in the omnidreams runtime's output layout.
        chunk = torch.randint(
            0, 255, (4, 3, 288, 512), dtype=torch.uint8, device="cuda",
        )
        packets: list = []
        num_frames, num_keyframes, encode_ms = encoder.encode_chunk_sync(
            chunk, force_keyframe=True, on_packet=packets.append,
        )
        assert num_frames == 4

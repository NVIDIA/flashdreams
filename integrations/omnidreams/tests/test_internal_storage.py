...
def test_omnidreams_paths_at_current_env() -> None:
    """Check the default/public omnidreams paths for the scrubbed test env.
    ...
    """
    from omnidreams import config

    # Mirrored chunk2 -> HF; unmirrored slugs fall through to s3.
    assert config.AVAILABLE_OMNIDREAMS_CHECKPOINT_PATHS["1view-vae-chunk2"].startswith(
        "https://huggingface.co/nvidia/"
    )
    assert config.AVAILABLE_OMNIDREAMS_CHECKPOINT_PATHS["1view-vae-chunk3"].startswith(
        "s3://"
    )

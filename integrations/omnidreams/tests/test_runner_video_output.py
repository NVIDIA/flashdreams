...
"""CPU tests for the OmniDreams runner's streaming video writer."""
...
import omnidreams.runner as runner_module
import pytest
import torch
...
def test_write_video_pads_odd_dimensions_for_yuv420p(
    ...
    runner_module._write_video(_canvas(), tmp_path / "odd.mp4", fps=24)
...
def test_write_video_reaps_ffmpeg_and_preserves_broken_pipe_diagnostic(
    ...
    with pytest.raises(RuntimeError, match="exit code 7.*encoder rejected frame"):
        runner_module._write_video(_canvas(), tmp_path / "broken.mp4", fps=24)

...
from .controls import CameraPoseIntegrator, KeyboardState
from .media import LingbotVideoTrack
...
class LingbotWebRTCSessionManager:
        ...
        *,
        ...
        if (
            ...
            self._send_json(
                ...
                {
                    "type": "busy",
                    "message": "Previous action is still being processed.",
                },
            )
            return
    ...
    async def _run_action_step(
        ...
        *,
        ...
        try:
            step_result = await managed_session.runtime.apply_action_and_generate(
                action_payload
            )

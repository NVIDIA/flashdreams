...
def tensor_chunk_to_rgb_frames(video_chunk: torch.Tensor) -> list[np.ndarray]:
    """Convert a Lingbot output tensor ``[T, C, H, W]`` to RGB uint8 frames.

    Matches the shipped configs' ``batch_shape=()`` single-rollout layout —
    the pipeline returns one chunk in ``[T, C, H, W]`` form with no leading
    batch / view axis.
    """
    if video_chunk.ndim != 4:
        raise ValueError(
            f"Expected video chunk with 4 dimensions [T, C, H, W], got {video_chunk.shape}"
        )

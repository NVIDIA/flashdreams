...
"""Onmidreams HDMap-conditioned I2V runner classes (single- + multi-view).

Pure implementation module. The per-slug ``*_RUNNER`` literals + the
``ONMIDREAMS_RUNNERS`` aggregating dict live in
:mod:`onmidreams.config`, alongside the matching
pipeline configs.

:meth:`OnmidreamsRunner.run` dispatches across three modes:
...
from onmidreams.hf import omni_dreams_hf_repo, omni_dreams_hf_url
...
EXAMPLE_DATA_DIR_S3 = "s3://flashdreams/assets/example_data/onmidreams"
"""Internal-team source for both views; also the external fallback for
multi-view (no HF mirror yet)."""

EXAMPLE_DATA_DIR_LOCAL = _REPO_ROOT / "assets/example_data/onmidreams"
...
def _ensure_s3_example_data_synced(
    ...
    sync_s3_dir_to_local(
        ...
        desc="Syncing onmidreams example data from S3",
    )
...
class OnmidreamsRunnerConfig(RunnerConfig):
    """Runner config covering every shipped Onmidreams variant.
    ...
    _target: type = field(default_factory=lambda: OnmidreamsRunner)
    ...
    output_fps: int = 30
    """Output video frame rate. Onmidreams was trained at 30fps."""
...
class OnmidreamsRunner(Runner[OnmidreamsRunnerConfig, OnmidreamsPipeline]):
    """Streaming HDMap-conditioned I2V driver."""

    config: OnmidreamsRunnerConfig

    def run(self) -> None:
        """Drive the Onmidreams AR rollout to completion."""
        cfg = self.config
    ...
    def _rollout_and_save(
        self, *, cache: OnmidreamsPipelineCache, num_views: int
    ) -> None:
    ...
    def _num_views(self) -> int:
        ...
        ``OnmidreamsPipeline.__init__`` divides ``transformer.config.num_views``

...
function logEvent(message, { source = "server", level = "info" } = {}) {
  const consoleMessage = `[Onmidreams WebRTC][${source}] ${message}`
  if (level === "error") {
...
function renderMetrics() {
  ...
  modelValue.textContent = metrics.model || "Onmidreams"
}
...
async function dumpPeerStats(reason) {
  ...
  try {
    ...
    console.group(`[Onmidreams WebRTC] peer stats: ${reason}`)
    for (const report of stats.values()) {
    ...
    console.warn("[Onmidreams WebRTC] getStats failed", error)
  }
}

...
"""User-facing configs for Onmidreams.

Hosts both the pre-built :class:`OnmidreamsPipelineConfig` literals
and the per-slug :class:`OnmidreamsRunnerConfig` literals that drive
...
from onmidreams.hf import omni_dreams_hf_url
from onmidreams.pipeline import (
    OnmidreamsPipelineConfig,
)
from onmidreams.runner import OnmidreamsRunnerConfig
from onmidreams.transformer import CosmosTransformerConfig
from onmidreams.transformer.impl.network import (
...
_INTERNAL_ONMIDREAMS_CHECKPOINT_PATHS: dict[str, str] = {
    "1view-pshuffle-chunk4": "s3://flashdreams/assets/checkpoints/onmidreams/16N@cosmos_v2_2b_SF_res720p_30fps_i2v_hdmap_chunk4_pixel_shuffle_resume.pt",
    "1view-vae-chunk2": "s3://flashdreams/assets/checkpoints/onmidreams/32n_cosmos_v2_2b_SF_res720p_30fps_i2v_hdmap_chunk2_vae_encode_189f_loc6_sft_urban_stationary_mixed_gcp_student_resume.pt",
    "1view-vae-chunk3": "s3://flashdreams/assets/checkpoints/onmidreams/32n_cosmos_v2_2b_SF_res720p_30fps_i2v_hdmap_chunk3_vae_encode_loc6_gcp.pt",
    "4view-pshuffle-chunk4": "s3://flashdreams/assets/checkpoints/onmidreams/32n_cosmos_v2_2b_SF_4view_res720p_fps30_chunk4_i2v_hdmap_pixel_shuffle_loc8st2_gcp.pt",
    "4view-vae-chunk4": "s3://flashdreams/assets/checkpoints/onmidreams/32n_cosmos_v2_2b_SF_4view_res720p_fps30_chunk4_i2v_hdmap_vae_encoding_loc8st2_gcp.pt",
    "1view-diffusion-forcing-chunk2": "s3://flashdreams/assets/checkpoints/onmidreams/16N@causal_cosmos2_2B_res720p_30fps_hdmap_hdmap_pretrained_chunk2_vae_mads1m_1080p@20260225100739_000010600.pt",
    "1view-bidirectional-chunk48": "s3://flashdreams/assets/checkpoints/onmidreams/32N@teacher_cosmos2_2B_res720p_30fps_hdmap_vae_mads1m_189frames_1080p@20260309090017_000005000.pt",
}
...
_PUBLIC_ONMIDREAMS_CHECKPOINT_PATHS: dict[str, str] = {
...
AVAILABLE_ONMIDREAMS_CHECKPOINT_PATHS: dict[str, str] = (
    dict(_INTERNAL_ONMIDREAMS_CHECKPOINT_PATHS)
    if use_internal_storage()
    else {
        **_INTERNAL_ONMIDREAMS_CHECKPOINT_PATHS,
        **_PUBLIC_ONMIDREAMS_CHECKPOINT_PATHS,
    }
)
"""Resolved at module import; set ``FLASHDREAMS_INTERNAL_STORAGE`` first."""

SV_2STEPS_CHUNK2_LOC6_LIGHTVAE_LIGHTTAE = OnmidreamsPipelineConfig(
    recipe_name="onmidreams-sv-2steps-chunk2-loc6-lightvae-lighttae",
    ...
    diffusion_model=DiffusionModelConfig(
        ...
        transformer=CosmosTransformerConfig(
            ...
            checkpoint_path=AVAILABLE_ONMIDREAMS_CHECKPOINT_PATHS["1view-vae-chunk2"],
            batch_shape=(1,),
            num_views=1,
            len_t=2,
...
SV_2STEPS_CHUNK2_LOC6_LIGHTVAE_LIGHTTAE_PERF = cast(
    ...
    derive_config(
        ...
        recipe_name="onmidreams-sv-2steps-chunk2-loc6-lightvae-lighttae-perf",
...
SV_2STEPS_CHUNK2_LOC6_VAE_VAE = cast(
    ...
    derive_config(
        SV_2STEPS_CHUNK2_LOC6_LIGHTVAE_LIGHTTAE,
        recipe_name="onmidreams-sv-2steps-chunk2-loc6-vae-vae",
...
SV_2STEPS_CHUNK3_LOC6_VAE_VAE = cast(
    ...
    derive_config(
        ...
        recipe_name="onmidreams-sv-2steps-chunk3-loc6-vae-vae",
        diffusion_model=dict(
            transformer=dict(
                checkpoint_path=AVAILABLE_ONMIDREAMS_CHECKPOINT_PATHS[
                    "1view-vae-chunk3"
                ],
                len_t=3,
            ),
...
SV_2STEPS_CHUNK4_LOC8_PSHUFFLE_LIGHTTAE = cast(
    ...
    derive_config(
        SV_2STEPS_CHUNK2_LOC6_LIGHTVAE_LIGHTTAE,
        recipe_name="onmidreams-sv-2steps-chunk4-loc8-pshuffle-lighttae",
        image_encoder=dict(checkpoint_path=AVAILABLE_WAN_VAE_CHECKPOINT_PATHS["vae"]),
...
MV_2STEPS_CHUNK4_LOC8_PSHUFFLE_LIGHTTAE = cast(
    ...
    derive_config(
        ...
        recipe_name="onmidreams-mv-2steps-chunk4-loc8-pshuffle-lighttae",
        diffusion_model=dict(
            transformer=dict(
...
SV_35STEPS_CHUNK2_LOC24_COSMOS2_2B_RES720P_30FPS_HDMAP_VAE_MADS1M = OnmidreamsPipelineConfig(
    recipe_name="onmidreams-sv-35steps-chunk2-loc24-cosmos2-2b-res720p-30fps-hdmap-vae-mads1m",
    text_encoder=CosmosReason1TextEncoderConfig(),
    image_encoder=WanVAEEncoderConfig(
        checkpoint_path=AVAILABLE_WAN_VAE_CHECKPOINT_PATHS["vae"],
    ),
...
"""Teacher: onmidreams diffusion-forcing causal AR (2B / 720p / chunk2 UniPC).
...
SV_35STEPS_CHUNK48_LOC48_COSMOS2_2B_RES720P_30FPS_HDMAP_VAE_MADS1M = cast(
    ...
    derive_config(
        ...
        recipe_name="onmidreams-sv-35steps-chunk48-loc48-cosmos2-2b-res720p-30fps-hdmap-vae-mads1m",
        diffusion_model=dict(
            seed=1,
            context_noise=0,
            transformer=dict(
...
"""Teacher: onmidreams bidirectional (single-view / 2B / 720p / chunk48 UniPC).
...
EXPERIMENT1_BASELINE = cast(
    ...
    derive_config(
        ...
        recipe_name="onmidreams-experiment1-baseline",
    ),
)  # ty:ignore[redundant-cast]
...
EXPERIMENT1_SKIP_FINALIZE_KV_CACHE = cast(
    ...
    derive_config(
        SV_2STEPS_CHUNK2_LOC6_LIGHTVAE_LIGHTTAE_PERF,
        recipe_name="onmidreams-experiment1-skip-finalize-kv-cache",
        diffusion_model=dict(
...
EXPERIMENT1_SKIP_FINALIZE_KV_CACHE_NOISE350 = cast(
    ...
    derive_config(
        ...
        recipe_name="onmidreams-experiment1-skip-finalize-kv-cache-noise350",
        diffusion_model=dict(
...
EXPERIMENT1_SKIP_FINALIZE_KV_CACHE_NOISE250 = cast(
    ...
    derive_config(
        EXPERIMENT1_SKIP_FINALIZE_KV_CACHE,
        recipe_name="onmidreams-experiment1-skip-finalize-kv-cache-noise250",
        diffusion_model=dict(
...
EXPERIMENT1_SKIP_FINALIZE_KV_CACHE_NOISE150 = cast(
    ...
    derive_config(
        EXPERIMENT1_SKIP_FINALIZE_KV_CACHE,
        recipe_name="onmidreams-experiment1-skip-finalize-kv-cache-noise150",
        diffusion_model=dict(
...
EXPERIMENT1_SKIP_FINALIZE_KV_CACHE_NOISE100 = cast(
    ...
    derive_config(
        EXPERIMENT1_SKIP_FINALIZE_KV_CACHE,
        recipe_name="onmidreams-experiment1-skip-finalize-kv-cache-noise100",
        diffusion_model=dict(
...
ONMIDREAMS_CONFIGS: dict[str, OnmidreamsPipelineConfig] = {
    cfg.recipe_name: cfg
    for cfg in (
        SV_2STEPS_CHUNK2_LOC6_LIGHTVAE_LIGHTTAE,
        SV_2STEPS_CHUNK2_LOC6_LIGHTVAE_LIGHTTAE_PERF,
...
"""All shipped Onmidreams variants, keyed by ``recipe_name``."""
...
RUNNER_SV_2STEPS_CHUNK2_LOC6_LIGHTVAE_LIGHTTAE = OnmidreamsRunnerConfig(
...
RUNNER_SV_2STEPS_CHUNK2_LOC6_LIGHTVAE_LIGHTTAE_PERF = OnmidreamsRunnerConfig(
...
RUNNER_SV_2STEPS_CHUNK2_LOC6_VAE_VAE = OnmidreamsRunnerConfig(
...
RUNNER_SV_2STEPS_CHUNK3_LOC6_VAE_VAE = OnmidreamsRunnerConfig(
...
RUNNER_SV_2STEPS_CHUNK4_LOC8_PSHUFFLE_LIGHTTAE = OnmidreamsRunnerConfig(
...
RUNNER_MV_2STEPS_CHUNK4_LOC8_PSHUFFLE_LIGHTTAE = OnmidreamsRunnerConfig(
...
RUNNER_SV_35STEPS_CHUNK2_LOC24_COSMOS2_2B_RES720P_30FPS_HDMAP_VAE_MADS1M = OnmidreamsRunnerConfig(
...
RUNNER_SV_35STEPS_CHUNK48_LOC48_COSMOS2_2B_RES720P_30FPS_HDMAP_VAE_MADS1M = OnmidreamsRunnerConfig(
...
RUNNER_EXPERIMENT1_BASELINE = OnmidreamsRunnerConfig(
...
RUNNER_EXPERIMENT1_SKIP_FINALIZE_KV_CACHE = OnmidreamsRunnerConfig(
...
RUNNER_EXPERIMENT1_SKIP_FINALIZE_KV_CACHE_NOISE350 = OnmidreamsRunnerConfig(
...
RUNNER_EXPERIMENT1_SKIP_FINALIZE_KV_CACHE_NOISE250 = OnmidreamsRunnerConfig(
...
RUNNER_EXPERIMENT1_SKIP_FINALIZE_KV_CACHE_NOISE150 = OnmidreamsRunnerConfig(
...
RUNNER_EXPERIMENT1_SKIP_FINALIZE_KV_CACHE_NOISE100 = OnmidreamsRunnerConfig(
...
ONMIDREAMS_RUNNERS: dict[str, RunnerConfig] = {
    cfg.runner_name: cfg
    for cfg in (
...
"""All shipped Onmidreams runners (single- and multi-view variants),

...
import pytest
import torch
from onmidreams.webrtc import session
from onmidreams.webrtc.session import (
    OnmidreamsInferenceRuntime,
...
def _fake_runtime_factory(config: OnmidreamsRuntimeConfig) -> object:
    del config
    return object()
...
def _build_fake_runtime() -> tuple[OnmidreamsInferenceRuntime, _FakeWrapper]:
    runtime = OnmidreamsInferenceRuntime(
        config=OnmidreamsRuntimeConfig(device="cpu", fps=30)
    )
...
async def test_session_manager_preload_runs_loopback_warmup_once(
    ...
    class _FakeRuntime:
        def __init__(self, config: OnmidreamsRuntimeConfig) -> None:
            self.config = config
    ...
    def _fake_runtime_factory(config: OnmidreamsRuntimeConfig) -> _FakeRuntime:
        nonlocal fake_runtime
        fake_runtime = _FakeRuntime(config)
        return fake_runtime

    async def _fake_loopback_warmup(
        self: OnmidreamsWebRTCSessionManager, *, num_chunks: int
    ) -> None:
        del self
        warmup_calls.append(num_chunks)

    monkeypatch.setattr(session, "OnmidreamsInferenceRuntime", _fake_runtime_factory)
    monkeypatch.setattr(
        OnmidreamsWebRTCSessionManager,
        "_run_loopback_warmup_session",
        _fake_loopback_warmup,
    )
    manager = OnmidreamsWebRTCSessionManager(
        runtime_config=OnmidreamsRuntimeConfig(device="cpu", warmup_chunks=2)
    )
...
async def test_loopback_warmup_drives_session_generation(
    ...
    monkeypatch.setattr(session, "OnmidreamsInferenceRuntime", _fake_runtime_factory)
    manager = OnmidreamsWebRTCSessionManager(
        runtime_config=OnmidreamsRuntimeConfig(
            device="cpu",
            fps=30,
            warmup_chunks=2,
        )
...
async def test_heartbeat_message_refreshes_client_liveness(
    ...
    monkeypatch.setattr(session, "OnmidreamsInferenceRuntime", _fake_runtime_factory)
    manager = OnmidreamsWebRTCSessionManager(
        runtime_config=OnmidreamsRuntimeConfig(device="cpu", warmup_chunks=0)
    )
    managed_session = session._ManagedOnmidreamsSession(

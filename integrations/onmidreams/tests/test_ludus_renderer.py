...
All tests are excluded from the default test run.  Run explicitly with::
    ...
    uv run pytest integrations/onmidreams/tests/test_ludus_renderer.py --runxfail -v
"""
...
CLIPGT_ZIP = REPO_ROOT / "assets" / "example_data" / "onmidreams" / "clipgt.zip"
...
# ---------------------------------------------------------------------------
# High-level: LudusRenderer wrapper used by the onmidreams pipeline
# ---------------------------------------------------------------------------
...
def test_ludus_renderer_wrapper_renders_frames(
    ...
    """
    from onmidreams.conditioning.renderer import (
        LudusRenderer,

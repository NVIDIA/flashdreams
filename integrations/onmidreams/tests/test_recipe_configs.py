...
"""Cheap import-time checks for the ``onmidreams`` plugin.
...
``onmidreams.config`` ``RUNNER_*`` literals exactly.
"""
...
import pytest
import tomllib
from onmidreams import config as config_mod
from onmidreams.config import ONMIDREAMS_RUNNERS
...
def test_runners_dict_is_non_empty() -> None:
    ...
    assert ONMIDREAMS_RUNNERS, "ONMIDREAMS_RUNNERS is empty"
...
def test_runner_name_mirrors_pipeline_recipe_name() -> None:
    ...
    drifted = {
        ...
        for slug, cfg in ONMIDREAMS_RUNNERS.items()
...
def test_runners_have_descriptions() -> None:
    ...
    empty = [
        slug for slug, cfg in ONMIDREAMS_RUNNERS.items() if not cfg.description.strip()
    ]
...
def test_entry_points_match_module_literals() -> None:
    ...
    module_slugs = set(ONMIDREAMS_RUNNERS)
    ...
    for slug, target in entries.items():
        ...
        assert module_name == "onmidreams.config", (
...
def test_entry_points_discoverable_when_installed() -> None:
    ...
    discovered = {ep.name for ep in eps if ep.value.startswith("onmidreams.")}
    if not discovered:
        pytest.skip("plugin not installed; run `uv sync` from the repo root first")
    assert discovered == set(ONMIDREAMS_RUNNERS), (
        f"discovered slugs ({sorted(discovered)}) != "
        f"plugin runners ({sorted(ONMIDREAMS_RUNNERS)})"
    )

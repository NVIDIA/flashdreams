...
- **`flashdreams-run`** (`flashdreams/scripts/cli.py`) is the unified CLI — one hyphenated console script (nerfstudio's `ns-train` shape) fronting a tyro subcommand union over the runner registry. `flashdreams-run --help` lists every runner; `flashdreams-run <runner> --help` shows every overridable field (including everything inside the wrapped `pipeline`). Use `--no-instantiate` to skip GPU work and just dump the resolved config. V2 application integrations use `flashdreams.applications_v2` instead and must not add a runner solely to bridge pipeline defaults into an application.

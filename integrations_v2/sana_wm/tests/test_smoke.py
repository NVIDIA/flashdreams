...
import sana_wm.impl.refiner as refiner_module
import torch
...
from sana_wm.impl.quant import (
...
from sana_wm.impl.refiner import SanaWMLTX2Refiner, _pack_latents, _unpack_latents
from sana_wm.impl.scheduler import (
...
def test_pipeline_uses_sana_diffusion_model() -> None:
    """Keep the public pipeline wired to explicit SANA-WM boundaries."""
...
def test_public_configs_are_pipeline_configs() -> None:
    assert SANA_WM_CONFIGS == {
        pipeline.name: pipeline
        for pipeline in (PIPELINE_SANA_WM_BIDIRECTIONAL, PIPELINE_SANA_WM_STREAMING)
    }


def test_streaming_pipeline_uses_sana_streaming_components() -> None:
    """Keep the streaming pipeline wired to explicit streaming boundaries."""
...
def test_sana_diffusion_config_instantiates_base_model() -> None:
    """Use FlashDreams' shared diffusion model."""
...
def test_pyproject_package_selection() -> None:
    ...
    pyproject = tomllib.loads(
        Path("integrations_v2/sana_wm/pyproject.toml").read_text(encoding="utf-8")
    )

...
def _load_bench_summary() -> ModuleType:
    path = Path("integrations_v2/sana_wm/tests/performance/bench_summary.py")
...
def _load_bench_sweep_summary() -> ModuleType:
    path = Path("integrations_v2/sana_wm/tests/performance/bench_sweep_summary.py")
...
def test_bidirectional_bench_script_rejects_low_precision_dry_run() -> None:
    ...
    result = subprocess.run(
        ["bash", "integrations_v2/sana_wm/tests/performance/bench.sh"],
        check=False,
        cwd=Path.cwd(),
        env=env,
        text=True,

...
Shared harness files live one level up in `integrations_v2/sana_wm/tests/`:
...
`bench.sh` runs `uv sync` from `integrations_v2/sana_wm/tests/`, reuses or clones the
...
```bash
cd integrations_v2/sana_wm/tests/performance
bash bench.sh
```

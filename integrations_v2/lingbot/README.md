...
```bash
uv pip install -e integrations_v2/lingbot
```
...
The `action2v-lingbot` entry point remains available as a temporary development
...
uv run --python 3.12 --package flashdreams-lingbot flashdreams-run \
  ...
  --output.fps 16 \
  --output.path outputs/action2v-lingbot-replay.mp4
```
...
from lingbot.impl.encoder.camctrl import CamCtrlInput
...
```bash
uv run --extra dev pytest integrations_v2/lingbot/tests
```

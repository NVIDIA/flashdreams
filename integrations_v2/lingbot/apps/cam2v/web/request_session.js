...
// This page was written for the v1 server's control protocol, which carried
// key presses, text events, heartbeats, and disconnects over the data channel
// as one message family. The v2 runtime's channel takes device events only
// (keyboard, mouse, focus, touch, ...) and rejects anything else with
// "Unsupported browser event type", so messages are translated here rather
// than in the shared server, where a page's protocol does not belong.
function toRuntimeEvent(payload) {
  const type = payload?.type
  if (type === "action") {
    const action = payload.action || {}
    if (action.event === "keydown" || action.event === "keyup") {
      return { type: "keyboard", key: action.key, pressed: action.event === "keydown" }
    }
    // "step" asked the v1 server to generate one chunk; the v2 runtime
    // generates continuously, so there is nothing to ask for.
    return null
  }
  if (type === "disconnect") {
    // Deliberately dropped rather than translated to the runtime's "close".
    // On v1 this released the peer connection; on v2 a close event ends the
    // whole application run, so refreshing the tab killed the server. The
    // server keeps the session for a reconnect on its own.
    return null
  }
  // The v2 runtime holds the peer connection open without a keepalive.
  if (type === "heartbeat") {
    return null
  }
  return payload
...
// Text events are session state rather than device input, so they travel over
// the session endpoint the application serves, not the data channel.
function postSessionEvent(payload) {
  const body = { event_id: payload.event_id, state: payload.state }
  ...
  fetch("/api/session/input", {
    ...
    .then((scene) => {
      modelAdapter?.onInitialScene?.(scene, modelContext)
    })
    .catch((error) => {
      logEvent(`event failed: ${error.message}`, { source: "client", level: "error" })
    })
  return true
}
...
function sendModelMessage(payload) {
  if (payload?.type === "event") {
    return postSessionEvent(payload)
  }
  if (!connected || !controlChannel || controlChannel.readyState !== "open") {
    return false
  }
  const event = toRuntimeEvent(payload)
  if (event === null) {
    return true
  }
...
async function loadModelAdapter() {
  ...
  // The v2 runtime serves this page out of the application's own web root, so
  // the adapter sits beside it and needs no discovery endpoint. /api/ui/config
  // still wins where a server provides one, keeping the v1 behaviour intact.
  let adapterModule = "/adapter.js?v=lingbot-v2-protocol-v4"
  stylesheetHrefs.add("/adapter.css")
  try {
    ...
    logEvent(`model UI config unavailable, using bundled adapter: ${error.message}`, { source: "client", level: "info" })
  }
  try {
    const module = await import(adapterModule)
...
function sendControlAction(action) {
  ...
  const event = toRuntimeEvent({ type: "action", action })
  if (event === null) {
    return false
  }
  inferenceInFlight = true
...
function sendHeartbeat() {
  ...
  // The v2 runtime keeps the peer connection open on its own and rejects a
  // heartbeat as an unknown event type, so nothing is sent.
...
// The server serves this page as soon as it binds a port, but it cannot
// negotiate a peer connection until the runtime opens a session -- which waits
// on checkpoint loading and kernel compilation, minutes on a cold start. An
// offer sent before then is refused with "WebRTC server is not open", and the
// three quick retries below expire long before a model is ready. /healthz
// publishes exactly that readiness, so wait for it instead of guessing.
async function waitForSessionOpen({ timeoutMs = 20 * 60 * 1000 } = {}) {
  ...
  while (Date.now() < deadline) {
    try {
      ...
      if (response.ok && (await response.json()).open === true) {
        if (announced) {
          logEvent("model ready", { source: "client" })
        }
        return true
      }
      unreachable = 0
    } catch {
      // Distinguish a server that is still loading -- which answers /healthz
      // with open:false -- from one that is not there at all. Polling a dead
      // server for twenty minutes looks identical to waiting for a slow one.
      unreachable += 1
      if (unreachable >= 5) {
    ...
    if (!announced) {
      announced = true
      setFlow("waiting for the model to load")
      logEvent("model still loading, connecting when ready...", { source: "client" })
    }
...
async function connectSession({ attemptsRemaining = 1 } = {}) {
  ...
  try {
    if (!(await waitForSessionOpen())) {
      throw new Error("the model did not finish loading in time")
    }

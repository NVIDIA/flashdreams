...
Usage (from repo root):
    uv run --no-sync python -m flashvsr.impl.grpc.uplift_client --input clip.mp4 --output out.mp4

    # Use unary chunk flow instead of streaming:
    uv run --no-sync python -m flashvsr.impl.grpc.uplift_client --input clip.mp4 --output out.mp4 --unary

    # Connect through a local tunnel:
    uv run --no-sync python -m flashvsr.impl.grpc.uplift_client --server localhost:50051 --input ...

    # Send JPEG inputs and rely on the server browser viewer for output:
    uv run --no-sync python -m flashvsr.impl.grpc.uplift_client --input clip.mp4 --input_format jpeg --display_only

    # Live-ingest stress mode: loop one video as 8-frame chunks at 30 fps:
    uv run --no-sync python -m flashvsr.impl.grpc.uplift_client --continuous --input clip.mp4
"""
...
from flashvsr.impl.grpc.protos import flashvsr_pb2 as pb2
from flashvsr.impl.grpc.protos import flashvsr_pb2_grpc as pb2_grpc

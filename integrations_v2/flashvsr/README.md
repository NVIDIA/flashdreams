...
```bash
PYTHONPATH=integrations_v2/flashvsr:flashdreams \
uv run --no-sync python -m flashvsr.impl.grpc.uplift_server \
    --port 50051 \
    --viewer_port 8080 \
    --cuda_graph \
    --attention_mode auto \
...
```bash
PYTHONPATH=integrations_v2/flashvsr:flashdreams \
uv run --no-sync python -m flashvsr.impl.grpc.uplift_client \
    --continuous \
...
CPU smoke + parity tests live under `integrations_v2/flashvsr/tests/`:

```bash
uv run --extra dev pytest integrations_v2/flashvsr/tests -v
```

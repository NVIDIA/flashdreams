...
from flashvsr.impl.transformer.network import FlashVSRDiTNetworkConfig
...
# Pipeline ``input_H`` / ``input_W`` are 704x1280 placeholders that callers
# derive to the input video's native dimensions before setup.


def _build_sparse_ratio_variant(sparse_ratio: float) -> FlashVSRPipelineConfig:
    """Build one ``sparse_ratio`` pipeline preset.

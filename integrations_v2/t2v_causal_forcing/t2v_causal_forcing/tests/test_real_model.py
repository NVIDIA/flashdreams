...
``T2V_CAUSAL_FORCING_REAL_MODEL_RUN`` is set, which costs the GPU job
milliseconds and still keeps the module imported and collected there. Run it
with a base temporary directory you can reach, then play the file::
...
_RUN_ENV = "T2V_CAUSAL_FORCING_REAL_MODEL_RUN"
...
"""Frames the first block decodes. The chunkwise config generates three latent
frames a block, and the Wan VAE decodes the first to one frame and each after it
to four, so the first block is 1 + 2 * 4."""

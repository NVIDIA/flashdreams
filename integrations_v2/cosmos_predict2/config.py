...
def _validate_t2v_total_blocks(total_blocks: int) -> None:
    if total_blocks != 1:
        raise ValueError("Cosmos Predict2 T2V --total-blocks must be 1.")
...
T2V_APPLICATION_DEFAULTS = T2VApplicationDefaults(
    ...
    pixel_height=720,
    pixel_width=1280,
    prompt="A robotic arm performs precision welding in a busy industrial workshop.",
    fps=16,
)
...
"""Cosmos Predict2 hooks exposed to the shared T2V application."""

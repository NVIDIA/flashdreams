...
- Every config sets `recipe_name: str` (a stable slug). The per-recipe `<NAME>_CONFIGS: dict[str, StreamInferencePipelineConfig]` dict is keyed by `recipe_name`. There is no central pipeline-config registry — pipelines are reachable via direct per-recipe imports. Recipes that ship a `Runner` (see §5) appear in the central `flashdreams.configs.runner_configs.BUILTIN_RUNNERS` dict, which is the only registry the `flashdreams-run` CLI dispatches over.
...
- **Register every variant** in `<NAME>_CONFIGS: dict[str, StreamInferencePipelineConfig]`. Build the dict from a tuple of literals using a `recipe_name`-keyed comprehension so a typo in either side is impossible:
  ...
  ```
- **No central pipeline-config registry.** Pipeline configs are reachable via direct per-recipe import (`from flashdreams.recipes.<name>.config import <NAME>_CONFIGS`). The only central registry is `BUILTIN_RUNNERS` (see "Runner layer + `flashdreams-run` CLI" below) — recipes that haven't been wrapped into a runner just don't appear in `flashdreams-run`.
